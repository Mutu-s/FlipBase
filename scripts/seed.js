...
async function main() {
  let flipBaseContract

  try {
    const contractAddresses = fs.readFileSync('./contracts/contractAddress.json', 'utf8')
    const { flipBaseContract: flipBaseAddress } = JSON.parse(contractAddresses)

    flipBaseContract = await ethers.getContractAt('FlipBase', flipBaseAddress)

    // Process #1
    await Promise.all(
      generateGameData(dataCount).map(async (game) => {
        await createGame(flipBaseContract, game)
      })
    )
        ...
        .map(async (_, i) => {
          ...
          await Promise.all(
            invitations.map(async (player) => {
              await sendInvitation(flipBaseContract, player)
            })
          )
        })
    )
    ...
    // Process #3
    await getGames(flipBaseContract)
    await getInvitations(flipBaseContract, 1)
    await getMyInvitations(flipBaseContract)
  } catch (error) {
