...
contract FlipBase is Ownable, ReentrancyGuard, ERC20 {

  uint256 private _totalGames;
  uint256 private _totalPlayers;
  uint256 private _totalInvitations;
  ...
  function invitePlayer(address receiver, uint256 gameId) public {
    ...
    invitation.id = _totalInvitations;
  ...
  function payout(uint256 gameId) public nonReentrant {
    ...
    uint256 totalStakes = game.stake * game.acceptees;
    uint256 platformFee = (totalStakes * servicePct) / 100;
    uint256 creatorFee = platformFee - (platformFee / 2);
    uint256 gameRevenue = totalStakes - platformFee - creatorFee;
    ...
    for (uint256 i = 0; i < game.numberOfWinners; i++) {
      uint256 payoutAmount = gameRevenue / game.numberOfWinners;
